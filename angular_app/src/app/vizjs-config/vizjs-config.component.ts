...
export class VizjsConfigComponent implements OnInit, OnDestroy {
	...
	authorized_folder = ["/L4_logistics/Conception/rapports_test/test/test_mongodb", "/public/Samples"];
	...
	onUpdateFolder(form: NgForm) {
		...
		//try to get ressources informations (test if there are ressources, the source folder is valid)
		...
		this.vizjsViewService.getResourcesInformation(
          ...
          success => {
            //print results
            console.log("Get resources information successfull ! => "+success);    
            ...
            if(success[0] !== undefined){
				...
				alert("Il n'y a aucune ressource disponible dans le dossier source sélectionné (pour vos critères de recherche), veuillez en choisir un autre");
			}
          },
          error => {
		...
		visualize.config({
	  	...
	  	visualize(function (v) {
	  		...
	  		instance.inputControls = v.inputControls({
		        resource: "/L4_logistics/Conception/rapports_test/test/test_mongodb/sous_dossier_num_2/config_custom_ic",//Ruptures_par_client_et_par_jour__Détails__1",//Ruptures_par_client_et_par_jour__Détails_",
		        container: "#ic",
	...
	exportCustomParamsToCSV(){
        ...
        function objectsToCSV(arr) {
		    ...
		    console.log("array before mapping to CSV STR\n"+array);
		    for(var x = 0; x < array[1].length; x++){
		    	console.log("elt "+x+" = "+array[1][x]);
		    }
		    console.log("array 1 infos | len :  "+array[1].length, "\nget props : "+ Object.getOwnPropertyNames(array[1]));
		    console.log("info dict array 1");
		    for(var key in array[1]){
                console.log("param key "+key+" = "+array[1][key]+" type of data = "+typeof array[1][key]);
            }

		    return array.map(row => {
		        return Object.values(row).map(value => {
		        	//if the value is an object with more than 1 value, concat all values inside one (separate with "|") to have a valid csv
		            if(value.length > 1 && typeof value !== "string"){
		            	return Object.assign(value).toString().replaceAll(",", "|");
		            }
	...
	dropHandler(ev) {
	  ...
	  fileReader.onload = function() {
	    ...
	    for(var i = 0; i < result[0].length; i++){
	    	var custom_params_val = [result[1][i]]; 
    		//if the value contains pipes, it's a collection (a special process must be made)
    		if( custom_params_val.toString().includes("|")){
    			//custom_params_val = [custom_params_val.toString().split("|").join(",")];
    			//console.log("new custom params val "+custom_params_val); 
    			custom_params[result[0][i].substring(1, result[0][i].length-1)] = [result[1][i]].toString().split("|");
    		}else{
