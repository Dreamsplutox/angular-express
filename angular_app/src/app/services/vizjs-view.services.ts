...
export class VizjsViewService {
  ...
  //return a promise with all resources found with the "resourcesSearch" vizjs feature (for our CustomSearch or regular search)
  getResourcesInformation(folder_choice:string, search_query:string, resources_types:any, username: string, password: string){
	visualize.config({
	    auth: {
	        name: username,
	        password:password
